...
type
    ...
    TLogFile = class(TWritingTextFile)
    private
      //cFile: System.Text;
      //cFilePath: String;
      //cOpened: Boolean;
...
End;
Function AddCurThreadIDToMessage(Const sMessage:String):String;
Begin
  Result:= SysUtils.Format('%.*d', [10, Windows.GetCurrentThreadId]) + c_Space + sMessage;
End;
...
Begin
  ...
  try
    ...
    Begin
      ...
      if sAddThreadID then
        cMessage:= AddCurThreadIDToMessage(cMessage);
      if sAddTimeStamp then
        cMessage:= AddTimeStampToMessage(cMessage, cTime);

      Self.Write(cMessage, sFinishLine);
    End;
  finally
    Self.ProcLeaveCriticalSection;
  end;
